...
- 어드민 GET 라우트는 토큰 없음 — URL 자체가 보안 layer. 추가 보안 필요 시 Vercel Authentication / IP 화이트리스트.
- 리드 DB에 없는 번호의 녹음 파일은 **읽지도·복사하지도 않음** (앱 자체의 개인정보 보호 원칙).
- **RTZR 서버 측 데이터** — 사용자 요청 DELETE API 는 RTZR 가 미제공 (조사 완료 2026-04-29).
  대신 RTZR 정책상 **전사 결과는 요청 후 3일 후 자동 삭제** (개발자 문서 명시).
  → 별도 삭제 연동 불필요. 어드민 업로드 성공 후 RTZR 서버 데이터는 자동 만료된다.
...
### 9.1 RTZR 데이터 처리 정책 확인 — ✅ 부분 완료 (2026-04-29)
- ✅ developers.rtzr.ai 문서 확인 — 전사 결과 **3일 후 자동 삭제** 명시
- ✅ DELETE API **미제공** 확인 (POST `/v1/transcribe`, GET `/v1/transcribe/{ID}` 만 존재)
- [ ] 오디오 원본 학습 사용 여부 / DPA 체결 가능 여부는 RTZR 영업 문의 필요 (운영 시점에 진행)

### 9.2 RTZR 삭제 연동 — ❌ N/A (RTZR API 미지원)
- RTZR 자체가 사용자 요청 삭제 API 를 제공하지 않으므로 코드 작업 불가.
- RTZR 의 자동 3일 만료 정책으로 충분 — 별도 연동 불필요.
- 향후 RTZR 가 DELETE API 를 추가하면 그때 `RtzrClient.delete(id)` 추가 + SttWorker 통합.
...
| 2026-04-29 | **§1~§5 일괄 완료**: Migration v1→v4 데이터 보존, idempotency, durationSec, 자동/수동 재시도, 미응답/부재중/거절 (callType), FileObserver, 배터리 예외, 재연락 Phase 1(구조화) + Phase 2(폰 로컬 알림). §6 은 RTZR API 미지원으로 N/A. |
