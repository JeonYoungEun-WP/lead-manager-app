...
> **2026-04-29 상태 업데이트**: §1 ~ §5 모두 구현 완료. §6 은 조사 결과 N/A.
> 자세한 변경은 git log + PRD 참조.

다음 개발 세션에서 Android Studio 가 있는 환경으로 돌아왔을 때 처리할 항목들.

리포 최신 상태: `main` 브랜치, 최신 커밋 `6c2c825` (푸시 완료).
미빌드 변경: `69f97ed` 에서 추가된 업로드 진단 필드 — 아래 §1 에서 적용.

---

## 🔴 1. 필수 — 미빌드 커밋 적용 + 업로드 실패 진단 — ✅ 완료

- [x] `69f97ed` 커밋 기준으로 빌드 & 폰 설치
- [x] ⚠️ ~~`fallbackToDestructiveMigration`~~ → **명시적 Migration(1,2)·(2,3)·(3,4)** 로 데이터 보존 확정
  - 실기 검증: leads 5건 + call_records 29건 그대로 유지됨
- [ ] (사용자 검증 항목) 다음 실 통화 발생 시 통화 상세 화면 하단 "어드민 업로드" 카드 표시 확인
- 실패 사유별 조치 가이드는 그대로 유효

---

## 🟡 2. PRD §7 단점 보완 — 업로드 안정성 — ✅ 완료

### 2.1 업로드 자동 재시도 — ✅ 완료
- [x] `UploadRetryWorker` 신규 — 단일 callId 재업로드 + 백오프 자동 처리
- [x] 정책: 즉시(SttWorker) → 1분 → 5분 → 30분 → 24시간 → 영구 FAILED
- [x] attempt 횟수는 WorkManager inputData 로 추적 (DB 스키마 변경 불요)

### 2.2 수동 재시도 버튼 — ✅ 완료
- [x] `CallDetailScreen` 의 `UploadStatusCard` 에 `uploadStatus=FAILED` 일 때 "재업로드" 버튼
- [x] 클릭 → `LeadRepository.retryUpload(ctx, callId)` → 즉시 큐잉
- [x] 실패 시 자동 재시도 체인 진입

### 2.3 중복 업로드 방지 — ✅ 완료
- [x] `POST /api/transcripts` 에서 (startedAt, clientCallId, agentName) 조합으로 idempotency 체크
- [x] 이미 존재하면 기존 record 반환 + `deduped: true` 플래그
- [x] prefix list 로 효율적 검색 (`transcripts/YYYY-MM/{startedAt}_*`)

### 2.4 통화 길이(durationSec) — ✅ 완료 (목록 표시는 추후 Phase)
- [x] `SttWorker` 가 `MediaMetadataRetriever` 로 추출 후 페이로드 포함
- [x] 백엔드 `TranscriptPayload.durationSec?: number` 추가
- [x] record JSON 에 보존 (path 포맷 변경 없음 — 하위 호환 유지)
- [x] 어드민 detail 헤더에 "길이 N분 NN초" 표시
- [ ] 어드민 list 항목 표시는 N+1 fetch 회피 위해 보류 (필요시 path v4 포맷으로 추가 가능)
...
### 3.1 녹음 파일 즉시 감지 — ✅ 완료
- [x] `RecordingsObserver` (ContentObserver) 신규 — SAF tree URI 변화 감시
- [x] 변화 감지 시 8초 디바운스 후 CallFolderScanWorker 즉시 큐잉
- [x] LeadApp 에서 DataStore URI 변화 자동 구독 → Observer (재)등록
- 15분 주기 워커는 백업 메커니즘으로 그대로 유지 (OS 가 ContentObserver 변화를 100% 알려주지 않을 수 있어서)
- ForegroundService 승격은 일단 보류 — 현 구조로 통화 종료 후 ~10초 내 처리됨

### 3.2 배터리 최적화 예외 — ✅ 완료
- [x] `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` 권한 Manifest 등록
- [x] SettingsScreen 에 "배터리 최적화 예외" 카드 추가
- [x] 현재 상태(✓ 예외 등록됨) 표시 + 미등록 시 다이얼로그 호출 버튼
- [x] 일부 OEM 미지원 시 일반 배터리 설정으로 폴백

---

## 🟣 4. 미응답·부재중·거절 통화도 리스트에 표시 — ✅ 완료 (4.1~4.3, 4.4 일부 보류)
...
## 🔵 6. PRD §9.2 — RTZR 삭제 연동 — ✅ 조사 완료 (2026-04-29) — N/A

**결론: 코드 작업 불필요.**

- ✅ RTZR 개발자 문서 확인 완료 — **DELETE 엔드포인트 미제공**
  - 공개 API: `POST /v1/transcribe`, `GET /v1/transcribe/{ID}` 두 개뿐
- ✅ RTZR 정책상 **전사 결과는 3일 후 자동 삭제** (developers.rtzr.ai 명시)
- → 사용자 요청 삭제 API 가 없으므로 `RtzrClient.delete()` 추가 불가
- → `rtzrDeleted` 플래그도 의미 없음 (삭제할 수 없으니 추적 불요)
- ✅ PRD §6 보안 섹션에 자동 만료 정책 명시 완료
- 향후 RTZR 가 DELETE API 를 추가하면 그때 재검토
